...
public class UMD extends Pane implements Base {
	...
	public UMD() {
		...
		File currentDir = new File(Main.getUserHome() + "/Sony/Video");
			...
			if (files[i] != null)
				try {
					...
					for (ISimpleInArchiveItem item : sa.getArchiveItems()) {
						if ("UMD_VIDEO\\ICON0.PNG".equals(item.getPath())) {
						...
						if (index == 2 && "UMD_VIDEO\\PIC0.PNG".equals(item.getPath())) {
							ByteArrayOutputStream bos = new ByteArrayOutputStream();
							ExtractOperationResult result = item.extractSlow(new ISequentialOutStream() {
								public int write(byte[] data) throws SevenZipException {
									try {
						...
						if (index == 2 && "UMD_VIDEO\\PIC1.PNG".equals(item.getPath())) {
							ByteArrayOutputStream bos = new ByteArrayOutputStream();
							ExtractOperationResult result = item.extractSlow(new ISequentialOutStream() {
								public int write(byte[] data) throws SevenZipException {
									try {
						...
						if (index == 2 && "UMD_VIDEO\\ICON1.PMF".equals(item.getPath())) {
							ByteArrayOutputStream bos = new ByteArrayOutputStream();
							ExtractOperationResult result = item.extractSlow(new ISequentialOutStream() {
								public int write(byte[] data) throws SevenZipException {
									try {
	...
	public void enter() {
		...
		try {
			...
			String vbsfile = "\"" + Utils.getJarFolder() + "umdvideo_bat.vbs\"";
			String batfile = "\"" + Utils.getJarFolder() + "umdvideo.bat\"";
